...
**LifeManga（漫画人生）** 是一款 **iPhone 上的原生 App**：你把日常照片丢进去，用 **OpenAI** 帮你画成 **日式漫画风格** 的图。

它把创作当成「做一本书」：**工程**里一页一页生成，能接上「上一张」保持画风；**角色库**里可以从一张照片拉出角色设定图，再在创作里引用；还能开 **故事模式** 先让 AI 写分镜剧本再画。**生成可以放到后台**，锁屏或切走 App 也能继续，做完会提醒。「发布」Tab 目前是占位，还没做导出分享。

简单说：**拍照 → 选风格和选项 → AI 出漫画页，带工程、角色和任务管理。**
